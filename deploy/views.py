...
from .forms import *
# custom function
from deploy.tar_file import make_tar
from deploy.md5 import md5sum
...
def salt_ajax_test(request):
    ...
    if request.method == 'POST':
        ...
        result_type = request.POST.get('result_type', None)
        if request.is_ajax():
            a = os.system('./IP.sh ' + ip + ' ')
            rst_all = Ajaxtest(ip, result_type, check_type, a)
            return HttpResponse(json.dumps(rst_all))
    else:
        raise Http404

def Ajaxtest(ip, result_type, check_type, a):
    '''
    定义ajax查询结果函数
    '''
    sret = a
