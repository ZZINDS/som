...
def login(request, redirect_field_name=REDIRECT_FIELD_NAME, authentication_form=AuthenticationForm):
    ...
    if request.method == "POST":
        #if request.POST.has_key('login'):      #python3 不兼容写法
        if "login" in request.POST:
            form = authentication_form(request, data=request.POST)
            if form.is_valid():
                if form.get_user() and form.get_user().is_active:
                    # Ensure the user-originating redirection url is safe.
                    # if not is_safe_url(url=redirect_to, host=request.get_host()):    #python3不兼容写法
                    #     redirect_to = resolve_url(djsettings.LOGIN_REDIRECT_URL)    #python3不兼容写法
